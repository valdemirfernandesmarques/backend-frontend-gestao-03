...
export default {
  ...
  setup() {
    ...
    const carregarVinculos = async () => {
      try {
        const response = await axios.get('https://wondrous-duckanoo-1d4650.netlify.app/api/professor-modalidade', { headers })
    ...
    const carregarProfessores = async () => {
      try {
        const response = await axios.get('https://wondrous-duckanoo-1d4650.netlify.app/api/professores', { headers })
    ...
    const carregarModalidades = async () => {
      try {
        const response = await axios.get('https://wondrous-duckanoo-1d4650.netlify.app/api/modalidades', { headers })
    ...
    async function submitForm() {
      try {
        if (editMode.value) {
          await axios.put(`https://wondrous-duckanoo-1d4650.netlify.app/api/professor-modalidade/${form.value.id}`, form.value, { headers })
        } else {
          await axios.post('https://wondrous-duckanoo-1d4650.netlify.app/api/professor-modalidade', form.value, { headers })
        }
        closeModal()
        carregarVinculos()
      } catch (err) {
    ...
    async function deletarVinculo(vinculo) {
      ...
      try {
        await axios.delete(`https://wondrous-duckanoo-1d4650.netlify.app/api/professor-modalidade/${vinculo.id}`, { headers })
        carregarVinculos()
      } catch (err) {
